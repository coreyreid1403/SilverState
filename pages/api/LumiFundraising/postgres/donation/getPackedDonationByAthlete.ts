...
export default async function handler(
  ...
  if(athleteId){
    const donations = await donationHelper.getPackedDonationsByAthlete(athleteId);
    res.status(200).json({ donations: donations, error: ''})
  } else{
    res.status(400).json({donations: [], error: 'Bad Request for getting packed donations by athlete'})
  }
}
